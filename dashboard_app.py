...
WHY = {
    ...
    2: ("The **Phillips Curve** posits an inverse trade-off between labour-market "
        "slack and inflation. Against the *level* of unemployment and the *level* "
        "of inflation it looks **flat or even positive** over 1948+, because two "
        "slow trends — the drifting natural rate **u\\*** and the great-inflation/"
        "disinflation path of **expected inflation** — swamp the demand signal "
        "(the 1970s stagflation is the classic culprit). The textbook fix "
        "(Friedman–Phelps; Gordon's triangle; Stock & Watson 1999) is the "
        "**expectations-augmented** form: the **unemployment gap** (u − u\\*, via "
        "an HP filter) drives the **change in inflation** (adaptive expectations). "
        "The slope −κ is the Phillips **slope**; the literature puts it near "
        "**−0.1 to −0.2** for both the US and Canada (Hazell–Herreño–Nakamura–"
        "Steinsson 2022). It remains **regime-dependent** (Principle 3): read the "
        "rolling correlation for the era-by-era story, and Principle 4 checks the "
        "link survives controlling for energy."),
